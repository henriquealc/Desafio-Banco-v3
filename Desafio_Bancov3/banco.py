...
# Definição da classe Historico para registrar as transações
class Historico:
    def __init__(self):
        self._transacoes = []
...
# Definição da classe abstrata Transacao com métodos abstratos
class Transacao(ABC):
    @property
    @abstractproperty
    def valor(self):
...
# Função para recuperar a primeira conta de um cliente (FIXME: atualmente não permite ao cliente escolher a conta)
...
# Função para criar uma nova conta corrente para um cliente
...
# Função principal que inicia o sistema e apresenta o menu para o usuário
def main():
    clientes = []
    contas = []
